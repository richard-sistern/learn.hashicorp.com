...
Let's run Vault in Dev Server mode. The simplest command to do this would be `vault server -dev`, but we want to set the initial root token to "root" and make the Vault server bind to all IP addresses. So, please run this instead:
...
Retrieve the health of the Vault server by running this command:

```shell
curl http://localhost:8200/v1/sys/health | jq
```

That should bring back a nicely formatted JSON document indicating that your server is initialised and unsealed.
...
Initialise the new server, indicating that you want to use one unseal key:
...
This will return the status of the server which should show that "Initialised" is "true" and that "Sealed` is "false".
...
Finally, log into the Vault UI with your root token.
...
You will get an error message because your token is not authorised to read any secrets yet. That is because Vault policies are "deny by default", meaning that a token can only read or write a secret if it is explicitly given permission to do so by one of its policies.
